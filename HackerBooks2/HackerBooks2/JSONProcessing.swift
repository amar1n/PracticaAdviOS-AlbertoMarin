...
func decode(book dict: JSONDictionary, context: NSManagedObjectContext) throws -> Book{
   ...
   // validate first
    ...
    // parsing
    let authors = Authors(parseCommaSeparated(string: extract(key: "authors")))
    let imgURL = URL(string: extract(key: "image_url"))!
    let pdfURL = URL(string: extract(key: "pdf_url"))!
    let tags = Tags(parseCommaSeparated(string: extract(key: "tags")))
    let title = extract(key: "title").capitalized
    
    return Book(title: title, authors: authors, tags: tags, coverUrl: imgURL, pdfUrl: pdfURL, context: context)
}
...
func decode(books dicts: JSONArray, context: NSManagedObjectContext) throws -> [Book]{
...
func decode(books dicts: JSONArray?, context: NSManagedObjectContext) throws -> [Book]{
    ...
    return try decode(books: ds, context: context)
}
